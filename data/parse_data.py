...
def create_csv_auth_gen(dict_obj):
    session_names = ["g0-train-vis", "g0-auth-vis", "g0-auth-two-vis"]

    with open("data_vis_auth.csv", "w", newline="") as file:
        ...
        for user in list(dict_obj.items()):
            ...
            for item in list(user[1].items()):
                ...
                if item[0] not in ["noteGenerateLag", "noteSpeed", "passSeq"]:
                    if item[1]["session"] == session_names[0]:
                        row.append(item[1]["hitRate"])
                    if item[1]["session"] == session_names[1]:
                        row.append(item[1]["hitRate"])
                        row.append(item[1]["block"][0]["passMetrics"]["hitRate"])
                    if item[1]["session"] == session_names[2]:
                        row.append(item[1]["hitRate"])
                        row.append(item[1]["block"][0]["passMetrics"]["hitRate"])
                        pass
...
def create_csv_aud_train(dict_obj):
    ...
    with open("data_vis_train.csv", "w", newline="") as file:
        csv_writer = csv.writer(file)
        ...
        for user in list(dict_obj.items()):
            ...
            for item in list(user[1].items()):
                ...
                if (item[0] not in ["noteGenerateLag", "noteSpeed", "passSeq"]) and (
                    item[1].get("session") == session_names[0]
                ):
...
if __name__ == "__main__":
    json_data = get_file_contents("data_vis.json")
